...
export default class Plan {
    ...
    updateStep(step) {
        ...
        if(step.end.time) {
            ...
            $(`#step-${step.id + 1} .text`).addClass("loaded");
            if(next.text().trim() == "") {
    ...
    show(name, sliding) {
        ...
        this.load(name, (title, content) => {
            ...
            renderer.link = (href, title, text) => {
                return `<a href="${href}" alt="${title}" target="_blank" rel="noopener noreferrer">${text}</a>`;
            };
                ...
                case "walk":
                    if (activity.duration == null) {
                        ...
                        if(track) {
                            ...
                            current.action = () => console.log("hello");
                        ...
                        this.getEndLocation(previous, start =>
                            this.train.calculate(start, current.text, date.format("YYYY-MM-DD"), current.end.time,
                                result => {
                                    ...
                                    current.action = () => {
                                        window.open(`https://www.sbb.ch/fr?date=%22${date.format("YYYY-MM-DD")}%22&moment=%22DEPARTURE%22`+
                                            "&stops=%5B%7B%22label%22%3A%22"+
                                            `${current.start.name}`+
                                            "%22%7D%2C%7B%22label%22%3A%22"+
                                            `${current.end.name}` +
                                            `%22%7D%5D&time=%22${current.start.time}%22`,
                                            '_blank',
                                            'noopener,noreferrer');
                                    }
                                    this.calculatePreviousEndTime(previous, current, callback);
                ...
                case "car":
                    if(activity.duration == null) {
                        this.route.calculate(previous.endLocation || previous.text, current.text, result => {
                            ...
                            current.action = () => {
                                window.open("https://www.google.com/maps/dir/?api=1&travelmode=driving" +
                                    `&origin=${current.start.name}`+
                                    `&destination=${current.end.name}`,
                                    '_blank',
                                    'noopener,noreferrer');
                            }
                            this.calculatePreviousEndTime(previous, current, callback);
